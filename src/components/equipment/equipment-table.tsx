...
export default function EquipmentTable() {
  ...
  const handleEdit = (eq: Equipment) => {
    ...
    setFormData({
      ...
      price: eq.price ?? '',
      status: eq.status,
      purchaseDate: eq.purchaseDate ? eq.purchaseDate.split('T')[0] : new Date().toISOString().split('T')[0],
      maintenanceIntervalDays: eq.maintenanceIntervalDays ?? '',
      nextMaintenance: eq.nextMaintenance ? eq.nextMaintenance.split('T')[0] : new Date().toISOString().split('T')[0],
