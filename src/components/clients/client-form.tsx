...
export function ClientForm({ mode, initialMember }: ClientFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    birthDate: initialMember?.birthDate ? initialMember.birthDate.split('T')[0] : "",
    ...
    monthlyPrice: initialMember ? String(initialMember.monthlyPrice ?? 0) : "",
  ...
  return (
    <form
      onSubmit={onSubmit}
      className={`${premium.formPanel} space-y-6 p-5 sm:p-6 lg:p-8`}
    >
      <div className={premium.formSection}>
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-[#5B6475]">
            Basic member identity and contact details.
          </p>
        </div>
        <PhotoUpload
          value={form.photoUrl}
      ...
      <div className={premium.formSection}>
        <div className="mb-5">
          <h2 className="text-lg font-bold text-[#0A1733]">
            Personal information
          </h2>
          <p className="mt-1 text-sm text-[#5B6475]">
            Keep this information accurate for member communications.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {field(
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  gender: e.target.value as NonNullable<Member["gender"]>,
                })
              }
              className={premium.formInput}
            >
          ...
          {field(
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, assignedTrainer: e.target.value })
              }
              className={premium.formInput}
      ...
      <div className={premium.formSection}>
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-[#5B6475]">
            Plan, goals and commercial information.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          {field(
            ...
            <select
              value={form.goal}
              onChange={(e) =>
                setForm({ ...form, goal: e.target.value as FitnessGoal })
              }
              className={premium.formInput}
            >
            ...
            <input
              ...
              onFocus={() => {
                if (form.monthlyPrice === "0") {
                  setForm({ ...form, monthlyPrice: "" });
                }
              }}
              onBlur={() => {
                if (!form.monthlyPrice.trim()) {
                  setForm({ ...form, monthlyPrice: "0" });
                }
              }}
              onChange={(e) =>
      ...
      <div className="flex flex-col-reverse gap-3 border-t border-[#E5EAF3] pt-6 sm:flex-row sm:justify-end">
        <button
          ...
          className={premium.formSecondaryBtn}
        >
          Cancelar
        </button>
        <button
          type="submit"
          disabled={isSubmitting}
          className={`${premium.pillBtn} disabled:cursor-not-allowed disabled:opacity-60`}
        >
          {isSubmitting
            ? "Guardando..."
            : mode === "create"
